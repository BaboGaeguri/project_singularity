#!/bin/bash
# Hylion v6 training (Newton-only)
# task: Velocity-Hylion-v0
# log: /tmp/hylion_v6_newton_train.log
...
PYTHONUNBUFFERED=1 LD_PRELOAD="/lib/aarch64-linux-gnu/libgomp.so.1" \
  nohup python "$TRAIN_SCRIPT" \
    ...
    --num_envs 4096 \
    --headless \
    --max_iterations 6000 > /tmp/hylion_v6_newton_train.log 2>&1 &

echo "[INFO] Newton v6 training started (PID: $!)"
echo "[INFO] Log: tail -f /tmp/hylion_v6_newton_train.log"
