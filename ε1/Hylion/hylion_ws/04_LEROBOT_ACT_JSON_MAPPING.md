# LeRobot ACT ↔ HYlion JSON 매핑 가이드

> **목표**: SO-ARM 101 Pro의 pick-and-place 학습에 사용한 LeRobot ACT 방식을 HYlion의 메인 명령 인터페이스로 통합

---

## 1. LeRobot ACT 데이터셋 구조 복습

### 1.1 저장 형식

```
datasets/so101_pick_place/
├── episode_0001/
│   ├── images/
│   │   ├── front_0000.jpg      ← 카메라 프레임
│   │   ├── front_0001.jpg
│   │   └── ...
│   ├── states.json              ← 관절 상태 시퀀스
│   └── texts.json               ← 태스크 설명 ("Pick up cup...")
├── episode_0002/
└── ...
```

### 1.2 상태 벡터 (states.json)

```json
{
  "joint_positions": [              // 각 timestep마다 6-DOF
    [0, 45, -60, 0, 0, 0],         // t=0: shoulder_pan, shoulder_lift, elbow_flex, wrist_flex, wrist_roll, gripper
    [5, 50, -55, 2, 0, 0],         // t=1
    [10, 55, -50, 5, 0, 100],      // t=2 (gripper 계산: 0~100, 0=열림, 100=닫힘)
    ...
  ],
  "gripper_states": [100, 100, 100, ...]  // 대안: 별도 필드
}
```

### 1.3 액션 청크 (Action Chunk)

ACT 모델은 **N개 미래 스텝의 관절 명령을 한 번에 예측**:

```
[입력]
- 현재 카메라 프레임 (RGB)
- 현재 관절 상태 (6-DOF + gripper)
- 언어 지시 ("Pick up the starbucks_cup")

[출력] — Action Chunk (shape: [chunk_size, 7])
- chunk_size = 100 (기본값, 약 5초 @50ms per step)
- 각 row: [θ1, θ2, θ3, θ4, θ5, θ6, gripper_command]
```

---

## 2. HYlion JSON v3 구조

### 2.1 핵심 필드

기본 원칙:
- 일상 대화/보행(intent!=pick_place)에서는 `so_arm_control` 비포함
- `pick_place`일 때만 SO-ARM 액션/ACT/IK 권장이 활성화

```json
{
  "motion_cmd": {
    "pipeline": "act_imitation | precoded_pick | rule_based",
    
    "so_arm_control": {
      "method": "act_sequence | inverse_kinematics | precoded",
      
      // ⭐ LeRobot ACT 모델이 생성할 출력
      "act_output": {
        "predicted_action_sequence": [
          {"joint_positions_deg": [0, 45, -60, 0, 0, 0], "gripper_effort": 0},
          {"joint_positions_deg": [5, 50, -55, 2, 0, 0], "gripper_effort": 0},
          ...
        ],
        "num_steps": 100,
        "total_duration_seconds": 5.0,
        "step_duration_ms": 50
      },
      
      // Fallback: IK로 생성할 목표
      "ik_target": {
        "position_xyz": [15, -5, 45],        // cm (로봇 기준)
        "orientation_rpy": [0, 90, 0]        // 도 (roll-pitch-yaw)
      },
      
      // Fallback: 사전 정의된 명령
      "fallback_joint_target": [0, 90, -90, 0, 0, 0],
      "fallback_gripper_effort": 60
    }
  }
}
```

### 2.2 SO-ARM 101 관절 순서 (중요!)

| 인덱스 | 관절명 | LeRobot ID | 범위 | 설명 |
|--------|--------|-----------|------|------|
| 0 | shoulder_pan | 1 | -180~180° | 수평 회전 |
| 1 | shoulder_lift | 2 | -90~90° | 상하 움직임 |
| 2 | elbow_flex | 3 | -150~0° | 팔꿈치 구부림 |
| 3 | wrist_flex | 4 | -90~90° | 손목 구부림 |
| 4 | wrist_roll | 5 | -180~180° | 손목 회전 |
| 5 | gripper | 6 | 0~100 | 그리퍼 (0=열림, 100=닫힘) |

> **주의**: 인덱스 번호가 모터 ID와 다름 (모터 ID = 1~6, 배열 인덱스 = 0~5)

---

## 3. 데이터 흐름도

### 3.1 학습 시 (Week 3~5)

```
[인간 텔레옵 수집]
  SO-ARM + 카메라 → LeRobot 데이터셋 (600 에피소드)
  
[LeRobot ACT 학습]
  DGX Spark에서 학습
  입력: (camera_frame, language_text)
  출력: action_chunk [N, 7]
  
[모델 저장]
  ~/lerobot/outputs/train/act_pick_place/checkpoints/last/
    → TensorRT로 변환 (Orin에서 실행 가능)
```

### 3.2 런타임 (Week 6+)

```
[사용자 음성 명령]
"컵 줄 수 있어?"

[LLM 처리]
Groq LLaMA 3.1 → JSON 생성
{
  "language_instruction": "Pick up the starbucks_cup",
  "target_object": "starbucks_cup",
  "motion_cmd.pipeline": "act_imitation"
}

[카메라 캡처 + ACT 추론]
현재 프레임 + "Pick up starbucks_cup" 
  → ACT 모델 (Orin TensorRT)
  → action_sequence [100, 7]

[로봇 실행]
for t in 0..99:
  set_joint_positions(action_seq[t, :6])
  set_gripper(action_seq[t, 6])
  sleep(50ms)
```

---

## 4. 구현 타임라인

### Week 0~1: JSON 스키마 확정 ✅ 완료

- `hylion_brain_v2.py`의 JSON 형식 정의
- `motion_cmd.so_arm_control` 구조 설계

### Week 2: IK 모듈 구현

```python
# lerobot/lerobot/common/robot_devices/arms/so_arm_ik.py
import ikpy.chain

so_arm_chain = ikpy.chain.Chain.from_urdf_file("so_arm_101.urdf")

def position_to_joints(target_xyz, target_rpy):
    target_frame = pose_to_frame(target_xyz, target_rpy)
    joint_angles = so_arm_chain.inverse_kinematics(target_frame)
    return joint_angles  # [θ1, θ2, θ3, θ4, θ5, gripper=0]
```

테스트:
```bash
# hylion_brain_v2.py 실행
[You]: "Put your hand at 20cm forward"
→ IK 계산 → JSON fallback_joint_target 자동 채움
```

### Week 3~5: ACT 학습 + 배포

```python
# lerobot/scripts/train.py (기존 코드)
lerobot-train \
    --policy.type=act \
    --dataset.repo_id=<user>/so101_pick_place \
    --output_dir=outputs/train/act_pick_place \
    --training.num_epochs=100

# 중간에 hylion_brain_v2.py 업데이트
# call_lerobot_act_model() 함수 구현
```

테스트:
```bash
# hylion_brain_v2.py 실행
[You]: "Pick up the starbucks cup"
→ LLM: language_instruction 추출
→ ACT 모델: action_sequence [100, 7] 생성
→ JSON: motion_cmd.so_arm_control.act_output 자동 채움
```

### Week 6+: 상하체 통합 + 운영

```bash
# 상체는 SmolVLA (pick-and-place)
# 하체는 Walking RL (보행)
# 둘 다 JSON의 motion_cmd에서 관리
```

---

## 5. 테스트 시나리오

### 시나리오 A: ACT 모델 로드 전 (지금)

```bash
python hylion_brain_v2.py

[You]: "Pick up the cup at x=20 y=-5"
→ LLM: motion_cmd.ik_target 생성
→ IK 모듈 (아직 없음): None 반환
→ JSON: fallback_joint_target = [0, 90, -90, 0, 0, 0]
→ 로봇: precoded 자세로 fallback
```

### 시나리오 B: ACT 모델 로드 후 (Week 3+)

```bash
python hylion_brain_v2.py

[You]: "Pick up the cup"
→ LLM: motion_cmd.language_instruction = "Pick up the cup"
→ ACT 모델: 카메라 + 텍스트 → 100-step 시퀀스 생성
→ JSON: motion_cmd.so_arm_control.act_output 자동 채움
→ 로봇: 100개 명령 재생 (5초, 50ms 간격)
```

---

## 6. JSON 예제

### 6.1 일상 대화/이동 (SO-ARM 제어 비활성)

```json
{
  "action_id": "abc-000",
  "timestamp": "2026-04-03T10:00:00Z",
  "state": "TALKING",
  "intent": "chat",
  "target_object": "None",
  "emotion": "neutral",
  "tts_text": "Hello! How can I help you today?",
  "perception": {
    "detected_objects": [],
    "target_position": {"x_cm": 0, "y_cm": 0, "z_cm": 0},
    "approach_angle_deg": 0
  },
  "motion_cmd": {
    "pipeline": "rule_based",
    "task_description": "Respond to greeting",
    "language_instruction": "Say hello and ask how to help"
  },
  "safety": {"collision_check": true, "force_limit_newtons": 30, "gripper_max_effort": 100, "emergency_stop": false},
  "gait_cmd": "None",
  "confidence": 0.95,
  "fallback": "Ask user to clarify request"
}
```

### 6.2 pick_place (SO-ARM 제어 활성)

```json
{
  "action_id": "abc-123",
  "timestamp": "2026-04-03T10:01:00Z",
  "state": "MANIPULATING",
  "intent": "pick_place",
  "target_object": "starbucks_cup",
  "emotion": "focused",
  "tts_text": "I am picking up the cup",
  "perception": {
    "detected_objects": [
      {"class": "starbucks_cup", "x_pixel": 300, "y_pixel": 200, "width": 80, "height": 120, "confidence": 0.93, "depth_cm": 45, "robot_x_cm": 15, "robot_y_cm": -5, "robot_z_cm": 45}
    ],
    "target_position": {"x_cm": 15, "y_cm": -5, "z_cm": 45},
    "approach_angle_deg": 0
  },
  "motion_cmd": {
    "pipeline": "act_imitation",
    "task_description": "Pick up the starbucks_cup and place it on table",
    "language_instruction": "Pick up the starbucks_cup",
    "so_arm_control": {
      "method": "act_sequence",
      "act_output": {
        "predicted_action_sequence": [
          {"joint_positions_deg": [0, 45, -60, 0, 0, 0], "gripper_effort": 0},
          {"joint_positions_deg": [5, 50, -55, 2, 0, 0], "gripper_effort": 0},
          {"joint_positions_deg": [10, 55, -50, 5, 0, 100], "gripper_effort": 100}
        ],
        "num_steps": 3,
        "total_duration_seconds": 0.15,
        "step_duration_ms": 50
      },
      "fallback_joint_target": [0, 90, -90, 0, 0, 0],
      "fallback_gripper_effort": 60
    }
  },
  "safety": {"collision_check": true, "force_limit_newtons": 30, "gripper_max_effort": 100, "emergency_stop": false},
  "gait_cmd": "stop",
  "confidence": 0.92,
  "fallback": "If ACT fails, use IK or precoded"
}
```

---

## 7. 주의사항

### 단위 통일

| 항목 | 단위 | 범위 | 비고 |
|------|------|------|------|
| joint_positions_deg | 도 (°) | 관절별 다름 | LeRobot 학습 데이터와 동일 |
| gripper_effort | % | 0~100 | 0=완전 열림, 100=완전 닫힘 |
| position_xyz | cm | -50~50 | 로봇 베이스 기준 |
| step_duration_ms | 밀리초 | 50 | 조절 불가 (LeRobot ACT는 50ms 고정) |

### 좌표계

```
로봇 기준계:
  X: 앞쪽 (+) / 뒤쪽 (-)
  Y: 오른쪽 (+) / 왼쪽 (-)
  Z: 위쪽 (+) / 아래쪽 (-)
  원점: 로봇 base_link의 중심
```

### 카메라 인식 방식: YOLO + ACT 통합

**현재 방식 (YOLO 우선):**
- YOLO로 객체 감지 → 픽셀 좌표 + 클래스 + 신뢰도
- 감지 결과를 LLM에 제공 → 텍스트 명령 생성
- ACT 모델은 별도 호출 (카메라 프레임 + 텍스트)

**LeRobot ACT의 카메라 사용:**
- ACT는 **end-to-end** 모델: 카메라 이미지 + 언어 지시 → 직접 액션 시퀀스 예측
- YOLO 없이도 동작 가능 (이미지에서 객체를 스스로 인식)
- YOLO는 **보조 감지**로 사용: ACT에 추가 컨텍스트 제공

**권장 통합 방식 (Week 3+):**
```python
# ACT 모델 입력
inputs = {
    "image": camera_frame,           # RGB 이미지
    "language_instruction": "Pick up the starbucks_cup",
    "object_bbox": yolo_bbox        # YOLO 감지 박스 (옵션)
}
action_chunk = act_model.predict(inputs)
```

**결론:** YOLO 유지 (LLM 컨텍스트용) + ACT에 통합. ACT는 YOLO 없이도 강력하지만, YOLO로 정확도 향상 가능.

---

## 8. FAQ

**Q: 왜 JSON에 `act_output`과 `fallback_joint_target`을 모두 포함?**

A: ACT 모델은 GPU 추론이 느릴 수 있으므로 (500ms~), 실시간성을 위해 fallback을 항상 준비해야 합니다.

**Q: `language_instruction`은 어디서 올까?**

A: LLM이 생성합니다. 예: 사용자 "컵 줄 수 있어?" → LLM이 JSON으로 "Pick up the starbucks_cup" 출력 → 이를 ACT 모델에 입력.

**Q: 카메라가 없으면?**

A: 시뮬레이션 카메라 프레임 또는 더미 프레임으로 대체. ACT는 언어만으로도 동작 예측 가능 (약간의 성능 저하).

**Q: 관절 순서를 틀리면?**

A: 로봇이 엉뚱한 방향으로 움직임. 필히 **Week 1 SO-ARM 실측으로 검증**.

## 7. ROS2 노드 구조

### 7.1 노드 분리

| 노드 | 파일 | 역할 | 입력 토픽 | 출력 토픽 |
|------|------|------|----------|----------|
| `brain_node` | `brain_node.py` | LLM 오케스트레이션 | `/hylion/user_input`, `/hylion/perception` | `/hylion/action_json`, `/hylion/tts` |
| `perception_node` | `perception_node.py` | 카메라 + YOLO 감지 | `/camera/image_raw` | `/hylion/perception` |
| `soarm_node` | `soarm_node.py` | SO-ARM 제어 생성 | `/hylion/action_json` | `/hylion/soarm_command` |

### 7.2 토픽 메시지

```python
# 사용자 입력 (String)
std_msgs/String: "Pick up the cup"

# 감지 결과 (String: JSON)
std_msgs/String: '[{"class": "starbucks_cup", "x_pixel": 300, ...}]'

# 액션 JSON (String: JSON)
std_msgs/String: '{"intent": "pick_place", "so_arm_control": {...}}'

# SO-ARM 명령 (String: JSON)
std_msgs/String: '{"type": "act_sequence", "action_sequence": [...]}'

# TTS (String)
std_msgs/String: "I am picking up the cup"
```

### 7.3 실행 예시

```bash
# 터미널 1: Brain Node
ros2 run hylion brain_node

# 터미널 2: Perception Node
ros2 run hylion perception_node

# 터미널 3: SO-ARM Node
ros2 run hylion soarm_node

# 터미널 4: 사용자 입력 퍼블리시
ros2 topic pub /hylion/user_input std_msgs/String "data: 'Pick up the cup'"
```

### 7.4 장점

- **모듈화**: 각 노드 독립 실행/디버깅 가능
- **확장성**: 노드 추가/교체 용이 (예: 다중 카메라)
- **실시간성**: 토픽으로 비동기 통신
- **ROS2 생태계**: RViz, rqt 등 도구 활용 가능
