...
### ROS2 토픽 인터페이스 (Orin ↔ NUC)

| 토픽 | 방향 | 타입 | 내용 |
|------|------|------|------|
| `/gait/cmd` | Orin → NUC | `std_msgs/String` | `walk_forward`, `stop`, `turn_left` 등 |
| `/gait/status` | NUC → Orin | `std_msgs/String` | `walking`, `stopped`, `error` 등 |
...
ros2 topic list   # Orin에서 NUC 토픽이 보이는지
```

**확인 항목**
- [ ] Ethernet 직결 ping 응답
- [ ] 같은 ROS_DOMAIN_ID에서 토픽 상호 인식
